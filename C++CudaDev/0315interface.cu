...
int main(int argc, char* argv[]) {
    ...
    for(int i = 0 ;i< 5 * 3; i ++){
        std::cout <<reinterpret_cast<float*>(inputImage.data)[i] <<",";
    }
    ...
    */
    ...
    float *h_data = new float[batchSize * channels * outputHeight * outputWidth]; 
    cudaMemcpy(h_data, tmpImage, outputHeight * outputWidth * channels * sizeof(float), cudaMemcpyDeviceToHost);
